...
geBoostTool::geBoostTool(int X, int Y, const m::SampleChannel* ch)
: Fl_Group(X, Y, 220, 20),
	ch      (ch)
{
	begin();
...
void geBoostTool::cb_setBoost()
{
	...
	if (Fl::event() == FL_DRAG)
		channel::setBoost(ch->index, dial->value());
	else 
...
void geBoostTool::cb_setBoostNum()
{
	...
	c::channel::setBoost(ch->index, u::math::dBtoLinear(atof(input->value())));
	static_cast<gdSampleEditor*>(window())->waveTools->updateWaveform();
}
...
void geBoostTool::cb_normalize()
{
	...
	c::channel::setBoost(ch->index, val); // it's like a fake user moving the dial 
	static_cast<gdSampleEditor*>(window())->waveTools->updateWaveform();
}
